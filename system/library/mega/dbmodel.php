...
class Dbmodel
{
	...
	const DB_PRODUCT_TABLE = 'mega_feed_pro_product';
	//added in version 1.2
	const DB_NEWS_TABLE = 'mega_feed_pro_news';
	//added in version 1.1
	...
	public function updateV1_1()
	{
		...
		/** modify old tables */
		$result = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . SELF::DB_GENERATOR_TABLE .
			"` LIKE '" . SELF::DB_GENERATOR_C_CURRENT_OFFSET . "'");
		if ($result->num_rows == 0)
		{
			$this->db->query("ALTER TABLE `" . DB_PREFIX . SELF::DB_GENERATOR_TABLE .
				"` ADD `" . SELF::DB_GENERATOR_C_CURRENT_OFFSET . "` INT NULL");
		}
		$result = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . SELF::DB_GENERATOR_TABLE .
			"` LIKE '" . SELF::DB_GENERATOR_C_ITEMS_COUNT . "'");
		if ($result->num_rows == 0)
		{
			$this->db->query("ALTER TABLE `" . DB_PREFIX . SELF::DB_GENERATOR_TABLE .
				"` ADD `" . SELF::DB_GENERATOR_C_ITEMS_COUNT . "` INT NULL");
		}
		$result = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . SELF::DB_GENERATOR_TABLE .
			"` LIKE '" . SELF::DB_GENERATOR_C_LAST_CREATION_DATE . "'");
		if ($result->num_rows == 0)
		{
			$this->db->query("ALTER TABLE `" . DB_PREFIX . SELF::DB_GENERATOR_TABLE .
				"` ADD `" . SELF::DB_GENERATOR_C_LAST_CREATION_DATE . "` DATETIME NULL");
		}
	}
	public function updateV1_2()
	{
		...
		$result = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . SELF::DB_NEWS_TABLE .
			"` LIKE '" . SELF::DB_NEWS_C_URL . "'");
		if ($result->num_rows == 0)
		{
			$this->db->query("ALTER TABLE `" . DB_PREFIX . SELF::DB_NEWS_TABLE . "` ADD `" .
				SELF::DB_NEWS_C_URL . "` VARCHAR(255) NOT NULL");
		}
	}
	public function setDbVersion()
	{
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSettingValue($this->extension_fullname, $this->extension_fullname .
