...
class Seznam
{
	const ZBOZI_PROD_BACKEND_URL =
		'https://www.zbozi.cz/action/SHOP_ID/conversion/backend';
	const ZBOZI_DEV_BACKEND_URL =
		'https://sandbox.zbozi.cz/action/TEST_ID/conversion/backend';
	private $registry;
	private $storeid = 0;
	...
	public function sklikConversionID()
	{
		$ids = $this->config->get($this->extension_fullname .
			'_adverts_sklik_conversion_id');
		return !empty($ids) && isset($ids[$this->storeid]) ? $ids[$this->storeid] :
			'undefined';
	}
	public function sklikConversionValue()
	{
		$values = $this->config->get($this->extension_fullname .
			'_adverts_sklik_conversion_value');
	...
	public function sklikConversionIsActive()
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_sklik_conversion_status');
	...
	{
		if ($this->sklikConversionIsActive() == 0 && $data['currency_code'] != 'CZK')
		{
			return;
		}
		...
		$totals = $data['currency_code'] != $this->default_store_currency ? $this->currency->convert($price,
		...
		$output .= "var seznam_value = " . ($this->sklikConversionValue() == '' ? $totals :
			$this->sklikConversionValue()) . ";";
		$output .= "</script>";
	...
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_sklik_retargeting_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	public function sklikFooterCodeSnippet()
	{
		if ($this->sklikRetargetingIsActive() == 0)
		{
			return;
		}
		$output = "";
	...
	{
		$keys = $this->config->get($this->extension_fullname .
			'_adverts_zbozi_secret_key');
		return !empty($keys) && isset($keys[$this->storeid]) ? $keys[$this->storeid] :
			'undefined';
	}
	public function zboziConversionIsActive()
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_zbozi_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	...
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_zbozi_standard');
		return (!empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1"))) || empty($statuses) ? 1 : 0;
	}
	public function zboziDebugModeIsActive()
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_zbozi_debug');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	...
	{
		if (($this->zboziConversionIsActive() == 0 || $this->zboziConversionIsStandard() ==
			0) && $data['currency_code'] != 'CZK')
		{
			return;
		}
		$requestUrl = "";
		...
		if (isset($this->session->data['shipping_method']))
		{
			...
			{
				$shipping_price = number_format((float)$this->tax->calculate($shipping_price, $shipping_tax_class_id),
					2, '.', '');
			}
			$shipping_price = $data['currency_code'] != $this->default_store_currency ? $this->currency->convert($shipping_price,
				$this->default_store_currency, $data['currency_code']) : $shipping_price;
		...
		if (!empty($data['products']))
		{
			foreach ($data['products'] as $product)
			{
				if (empty($product['option']))
				{
					if ($this->zboziDPHIsActive())
					{
						$price = number_format((float)$this->tax->calculate($product['price'], $product['tax_class_id']),
							2, '.', '');
					} else
					{
						$price = $product['price'];
	...
	{
		if ($this->zboziConversionIsActive() == 0 && $data['currency_code'] != 'CZK')
		{
			return;
		}
		$output = "";
