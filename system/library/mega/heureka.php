...
class Heureka
{
	...
	{
		$ids = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_cz_id');
		return !empty($ids) && isset($ids[$this->storeid]) ? $ids[$this->storeid] :
			'undefined';
	}
	public function heurekaCustomerCzIsActive()
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_cz_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	public function heurekaCustomerWidgetCzIsActive()
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_cz_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	...
	{
		$ids = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_cz_id');
		return !empty($ids) && isset($ids[$this->storeid]) ? $ids[$this->storeid] :
			'undefined';
	}
	public function heurekaCustomerWidgetCzPosition()
	{
		$positions = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_cz_position');
		return !empty($positions) && isset($positions[$this->storeid]) ? $positions[$this->storeid] :
			21;
	}
	public function heurekaCustomerWidgetCzTopMargin()
	{
		$margins = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_cz_top_margin');
		return !empty($margins) && isset($margins[$this->storeid]) ? $margins[$this->storeid] :
			60;
	}
	public function heurekaCustomerWidgetCzIsActiveOnMobile()
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_cz_mobile_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	public function heurekaCustomerWidgetCzMinScreenWidth()
	{
		$widths = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_cz_min_screen_width');
		return !empty($widths) && isset($widths[$this->storeid]) ? $widths[$this->storeid] :
			0;
	}
	public function heurekaCustomerSkID()
	{
		$ids = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_sk_id');
		return !empty($ids) && isset($ids[$this->storeid]) ? $ids[$this->storeid] :
			'undefined';
	}
	...
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_sk_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	...
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_sk_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	...
	{
		$positions = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_sk_position');
		return !empty($positions) && isset($positions[$this->storeid]) ? $positions[$this->storeid] :
			21;
	}
	...
	{
		$ids = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_sk_id');
		return !empty($ids) && isset($ids[$this->storeid]) ? $ids[$this->storeid] :
			'undefined';
	}
	...
	{
		$margins = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_sk_top_margin');
		return !empty($margins) && isset($margins[$this->storeid]) ? $margins[$this->storeid] :
			60;
	}
	...
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_sk_mobile_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	...
	{
		$widths = $this->config->get($this->extension_fullname .
			'_adverts_heureka_customer_widget_sk_min_screen_width');
		return !empty($widths) && isset($widths[$this->storeid]) ? $widths[$this->storeid] :
			0;
	}
	public function heurekaCustomerSuccessRequest($data)
	{
		if ($this->heurekaCustomerCzIsActive() == 0 && $this->heurekaCustomerSkIsActive
			() == 0)
		{
			return;
		}
		$requestUrl = "";
		if (in_array(strtolower($this->language->get('code')), array('cs', 'cs_cz')) &&
			$data['currency_code'] == 'CZK')
		{ //CZ
			$ID = $this->heurekaCustomerCzID();
			$requestUrl = $this::HEUREKA_URL_CZ;
		} elseif (in_array(strtolower($this->language->get('code')), array('sk', 'sk_sk')) &&
	...
	{
		if (($this->heurekaCustomerWidgetCzIsActive() == 0 && $this->heurekaCustomerWidgetSkIsActive
			() == 0) || !in_array(strtolower($this->language->get('code')), array(
			'cs',
			'cs_cz',
			'sk',
			'sk_sk')))
		...
		$output .= "var mega_heureka_widget_min_screen_width = " . intval($minScreenWidth) .
			";";
		$output .= "var mega_heureka_show_on_mobile = " . intval($onMobile) . ";";
		$output .= "</script>";
		$output .= "<!-- ./Heureka.cz/.sk : veryfied by customers widget -->";
		return $output;
	}
	public function heurekaCustomerWidget()
	{
		...
		$output .= "ho.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.heureka." .
			$domain_suffix . "/direct/i/gjs.php?n=wdgt&sak=' + widgetID;";
	...
	{
		$ids = $this->config->get($this->extension_fullname .
			'_adverts_heureka_conversion_cz_id');
		return !empty($ids) && isset($ids[$this->storeid]) ? $ids[$this->storeid] :
			'undefined';
	}
	...
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_heureka_conversion_cz_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	...
	{
		$ids = $this->config->get($this->extension_fullname .
			'_adverts_heureka_conversion_sk_id');
		return !empty($ids) && isset($ids[$this->storeid]) ? $ids[$this->storeid] :
			'undefined';
	}
	...
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_heureka_conversion_sk_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	public function heurekaConversionSuccessCodeSnippet($data)
	{
		if ($this->heurekaConversionCzIsActive() == 0 && $this->heurekaConversionSkIsActive
			() == 0)
		{
			return;
		}
		...
		if (!empty($data['products']))
		{
			foreach ($data['products'] as $product)
			{
				...
				$price = $data['currency_code'] != $this->default_store_currency ? $this->currency->convert($price,
					$this->default_store_currency, $data['currency_code']) : $price;
