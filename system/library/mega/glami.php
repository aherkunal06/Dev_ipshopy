...
class Glami
{
	...
	public function setPixels()
	{
		...
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_glami_pixel_id_status');
	...
	public function pixelHeaderCodeSnippet()
	{
		...
		foreach ($this->trackers as $country_code => $pixel)
		{
			...
			{
				$output .= "glami('create', '" . $pixel . "', '" . $country_code . "', '" . $country_code .
					"tracker');";
	...
	public function pixelSuccessCodeSnippet($data = array())
	{
		...
		foreach ($this->trackers as $country_code => $pixel)
		{
			if ($itterator == 0)
			{
				$totals = $data['currency_code'] != $this->default_store_currency ? $this->currency->convert($data['sub_totals'],
					$this->default_store_currency, $data['currency_code']) : $data['sub_totals'];
			...
			{
				$sub_totals = $this->convert_currency($data['sub_totals'], $data['currency_code'],
					self::$pixel_supported_countries[strtoupper($country_code)]['currency']);
	...
	public function pixelAddToCartCodeSnippet($data)
	{
		...
		foreach ($this->trackers as $country_code => $pixel)
		{
			if ($itterator == 0)
			{
				$price = $data['currency'] != $this->default_store_currency ? $this->currency->convert($data['price'],
					$this->default_store_currency, $data['currency']) : $data['price'];
			...
			{
				$price = $this->convert_currency($data['price'], $data['currency'], self::$pixel_supported_countries[strtoupper
	...
	public function reviewsMerchantID()
	{
		$ids = $this->config->get($this->extension_fullname .
			'_adverts_glami_reviews_merchant_id');
		return !empty($ids) && isset($ids[$this->storeid]) ? $ids[$this->storeid] :
			'undefined';
	}
	public function reviewsCountry()
	{
		$countries = $this->config->get($this->extension_fullname .
			'_adverts_glami_reviews_country');
		return !empty($countries) && isset($countries[$this->storeid]) && $countries[$this->storeid] !=
			"0" ? $countries[$this->storeid] : '';
	}
	public function reviewsIsActive()
	{
		$statuses = $this->config->get($this->extension_fullname .
			'_adverts_glami_reviews_status');
		return !empty($statuses) && isset($statuses[$this->storeid]) && in_array($statuses[$this->storeid],
			array("on", "1")) ? 1 : 0;
	}
	public function reviewsSuccessCodeSnippet($data = array())
	{
		if ($this->reviewsIsActive() == 0)
		{
			return;
		}
		$item_names = array();
		...
		if (empty($item_names) || empty($item_ids) || $this->reviewsCountry() == '')
		{
			return;
		}
		$output = "";
		...
		$output .= "glami_or('addParameter', 'merchant_id','" . $this->reviewsMerchantID() .
			"', '" . $this->reviewsCountry() . "');";
