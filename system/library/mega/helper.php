	...
	public static function getOptionData($product_id, $name, $options)
	{
		...
		if (!empty($options))
		{
			...
			foreach ($input_array as $input_array_member)
			{
				...
				{
					if ((int)$product_option_value_iterator['product_option_value_id'] == 0)
					{
						continue;
					}
					$temp_id = $input_array_member['id'] . '-' . $product_option_value_iterator['product_option_id'] .
						'_' . $product_option_value_iterator['product_option_value_id'];
					...
					$output_array = $input_array_member = array(
						"id" => $temp_id,
						"name" => $temp_name,
	...
	public static function transformPostDataToOptions($post_data, $product_options)
	{
		...
		foreach ($post_data as $o_key => $o)
		{
			foreach ($product_options as $po)
			{
				...
				if ($po['product_option_id'] == $o_key && $po['required'] == 1 && in_array($po['type'],
					array(
					'radio',
					'checkbox',
					'select')))
				{
					if (!empty($po['product_option_value']))
					{
						...
						{
							if (!is_array($o) && $po_v['product_option_value_id'] == $o)
							{ //radio, select
