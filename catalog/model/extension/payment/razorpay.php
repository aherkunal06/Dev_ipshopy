...
class ModelExtensionPaymentRazorpay extends Model
{
    ...
    // function updatePayment added on 23-06-2025 
    
    public function updatePayment($transaction_data) {
        // Create the razorpay_transaction table if it doesn't exist with order_ids_json
        $this->db->query("CREATE TABLE IF NOT EXISTS " . DB_PREFIX . "razorpay_transaction (
            ...
            is_parent_order tinyint(1) DEFAULT 0,
            order_ids_json TEXT DEFAULT NULL,
        ...
        // Prepare order_ids_json if provided
        $order_ids_json = null;
        if (isset($transaction_data['order_ids']) && is_array($transaction_data['order_ids'])) {
            $order_ids_json = $this->db->escape(json_encode($transaction_data['order_ids']));
        }

        // Build the query based on whether this is a parent order or not
        $query = "INSERT INTO " . DB_PREFIX . "razorpay_transaction SET 
            razorpay_payment_id = '" . $this->db->escape($transaction_data['razorpay_payment_id']) . "',
            razorpay_order_id = '" . $this->db->escape($transaction_data['razorpay_order_id']) . "',
            razorpay_signature = '" . $this->db->escape($transaction_data['razorpay_signature']) . "',
            merchant_order_id = '" . (int)$transaction_data['merchant_order_id'] . "',
            amount = '" . (float)$transaction_data['amount'] . "',
            currency = '" . $this->db->escape($transaction_data['currency']) . "',
            status = '" . $this->db->escape($transaction_data['status']) . "',
            date_added = '" . $this->db->escape($transaction_data['date_added']) . "'";
            
        // Add parent order details if available
        if (isset($transaction_data['parent_order_id'])) {
            $query .= ",
            parent_order_id = '" . $this->db->escape($transaction_data['parent_order_id']) . "',
            is_parent_order = '" . (int)$transaction_data['is_parent_order'] . "'";
        }

        // Add order_ids_json if available
        if ($order_ids_json !== null) {
            $query .= ",
