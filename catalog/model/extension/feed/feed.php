<?php
class ModelExtensionFeedFeed extends Model {
  public function getProducts($args) {
    $query = $this->db->query("SELECT p.product_id,p.quantity, p.image,p.model,p.ean,p.price, DATE_FORMAT(GREATEST(p.date_added, p.date_modified),'%Y-%m-%dT%TZ') as date_modified, pd.name,pd.description FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . $args['language_id'] . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . $args['store_id'] . "' ORDER BY p.sort_order, LCASE(pd.name)");
