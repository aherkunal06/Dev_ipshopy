...
class ModelExtensionTmdaccount extends Model {
	...
	public function getOrders($start = 0, $limit = 20) {
		...
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, os.name as status, o.date_added, o.total, o.currency_code, o.currency_value, op.quantity FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) LEFT JOIN ". DB_PREFIX ."order_product op on (o.order_id = op.order_id) WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND o.order_status_id > '0' AND o.store_id = '" . (int)$this->config->get('config_store_id') . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);
	...
	public function getTotalOrder($customer_id){
		$sql="select count(*) as total from `" . DB_PREFIX . "order` where customer_id='".$this->customer->getId()."'  AND order_status_id = 1";
		$query=$this->db->query($sql);
	...
	public function getprofile(){
		$sql="select * from " . DB_PREFIX . "profileimage where customer_id='".$this->customer->getId()."'";
		$query=$this->db->query($sql);
		return $query->row;
	}

	public function editProfileImage($data) {

		$this->db->query("DELETE FROM " . DB_PREFIX . "profileimage WHERE customer_id = '" . (int)$this->customer->getId() . "'");

		if (isset($data['image'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "profileimage SET image = '" . $this->db->escape($data['image']) . "', customer_id = '" . (int)$this->customer->getId() . "'");
		}
	}
}
