...
// 			$earned = round($sub_total * 0.05, 2);
// 			$this->model_ipoffer_offer->setReferralConversionAndEarned($data['tracking'], $earned);
// 			if (isset($data['customer_id']) && method_exists($this->model_ipoffer_offer, 'addReferralConversion')) {
// 				$this->model_ipoffer_offer->addReferralConversion($data['tracking'], $order_id, $data['customer_id'], $earned);
// 			}
		return $order_id;
	}
    //     public function addOrderParent($data) {
    //     $this->db->query("INSERT INTO `" . DB_PREFIX . "order_parent` 
    //         (order_ids, total, courier_charges) 
    //         VALUES (
    //             '" . $this->db->escape($data['order_ids']) . "', 
    //             '" . (float)$data['total'] . "', 
    //             '" . (float)$data['courier_charges'] . "'
    //         )");
    //         $parent_order_id = $this->db->getLastId();
    //         return $parent_order_id;
    // }

    public function addOrderParent($data) {
        // Generate a unique parent_order_id (e.g., timestamp + random string)
        $parent_order_id = date('YmdHis') . '-' . substr(md5(uniqid(rand(), true)), 0, 8);

        $this->db->query("INSERT INTO " . DB_PREFIX . "order_parent SET " .
            "parent_order_id = '" . $this->db->escape($parent_order_id) . "', " .
            "order_ids = '" . $this->db->escape($data['order_ids']) . "', " .
            "courier_charges = '" . (float)$data['courier_charges'] . "', " .
            "total = '" . (float)$data['total'] . "', " .
            "date_added = NOW()");

        $id = $this->db->getLastId();

        // Log the parent order creation
        $this->log->write('Created parent order: ID=' . $id . ', parent_order_id=' . $parent_order_id . ', order_ids=' . $data['order_ids']);

        return $parent_order_id;
    }
    
    public function getOrderParent($parent_order_id) {
		// First try to get by ID (integer)
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_parent WHERE id = '" . (int)$parent_order_id . "'");
		
		// If no result, try to get by parent_order_id (string)
		if (!$query->num_rows) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_parent WHERE parent_order_id = '" . $this->db->escape($parent_order_id) . "'");
		}
	...
	public function createInvoiceNo($order_id) {
		...
		if ($order_query->num_rows && !$order_query->row['invoice_no']) {
			$query = $this->db->query("SELECT MAX(invoice_no) AS invoice_no FROM `" . DB_PREFIX . "order` WHERE invoice_prefix = '" . $this->db->escape($order_query->row['invoice_prefix']) . "'");
			...
			return $invoice_no;
		} elseif ($order_query->row['invoice_no']) {
			return $order_query->row['invoice_no']; // Already exists
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		// Check if new status is "Complete"
		if (in_array((int)$order_status_id, (array)$this->config->get('config_complete_status'))) {
			$this->load->model('account/order');
		
			$order_info = $this->getOrder($order_id); // ✅ FIXED
			...
			if ($order_info) {
				...
				$invoice_path = $this->load->controller('account/order/invoicePDF', (int)$order_id);
		
				$this->model_account_order->InvoicePDF($order_id, $invoice_no, $invoice_path);
			}
		}
	}
