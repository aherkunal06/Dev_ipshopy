...
class ControllerExtensionModuleTmdAccount extends Controller {
	public function index($setting) {
		...
		if(!empty($accountlables[$this->config->get('config_language_id')]['customlabel'])){
		$data['text_customlinks']	= $accountlables[$this->config->get('config_language_id')]['customlabel'];
		} else {
		$data['text_customlinks'] = $this->language->get('text_customlinks');
		}
		...
		$data['text_affilate'] = $this->language->get('text_password');
		}
		...
		$data['tmdaccounticons'] = array();

		$tmdicons_infos =$this->config->get('tmdaccount_icon');
		if (isset($tmdicons_infos)){
			foreach($tmdicons_infos as $tmdicons_info){
				
				$dd=$tmdicons_info['tmdaccountdescription'];
				
				
				if(!empty($dd[$this->config->get('config_language_id')]['text'])){
					$text	= $dd[$this->config->get('config_language_id')]['text'];
				} else {
					$text = $this->language->get('text_text');
				}
				if(!empty($tmdicons_info['tmdaccounticonrouting'][$this->config->get('config_language_id')]['routing'])) {
					$routing	= $tmdicons_info['tmdaccounticonrouting'][$this->config->get('config_language_id')]['routing'];
				} else {
					$routing ='';
				}
				...
				$data['tmdaccounticons'][	] = array(
				'icon' => $tmdicons_info['icon'],
				'text' => $text,
				'routing' => $routing,
				'sort_order' => $tmdicons_info['sort_order'],
				'status' => $tmdicons_info['status'],
				);
		...
		foreach ($data['tmdaccounticons'] as $key => $value) {
			$sort_order[$key] = $value['sort_order'];
		}

		array_multisort($sort_order, SORT_ASC, $data['tmdaccounticons']);
        ...
        if($accounttemplate=='1'){
		  ...
		  return $this->load->view('extension/module/tmdaccount1', $data);
        } else {
         return $this->load->view('extension/module/tmdaccount', $data);
        }
	}
