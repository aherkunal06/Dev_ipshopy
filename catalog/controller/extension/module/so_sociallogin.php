...
class ControllerExtensionModuleSoSociallogin extends Controller {
	...
	public function FacebookLogin() {
		...
		// ✅ ADDITION: Prevent direct access without Facebook login flow
    	if (!isset($_GET['state']) || !isset($_GET['code'])) {
...
// 	commented on 01-04-2025 TwitterLogin(), TwitterToken()
...
// 	added new function on 01-04-2025
	public function TwitterLogin() {
		$this->load->model('setting/setting');
		$setting = $this->model_setting_setting->getSetting('so_sociallogin');
		...
		$connection = new TwitterOAuth($twitapikey, $twitsecretapi);
		$callback_url = $this->url->link('extension/module/so_sociallogin/TwitterToken', '', 'SSL');
	
		// Debugging: Print callback URL to confirm
	...
	public function TwitterToken() {
		...
		if (!empty($this->request->get['oauth_verifier']) && 
			!empty($this->session->data['oauth_token']) && 
			!empty($this->session->data['oauth_token_secret'])) {
	
			$twitteroauth = new TwitterOAuth(
				$setting['so_sociallogin_twitapikey'],
				$setting['so_sociallogin_twitsecretapi'],
			...
			if (!isset($user_info->id)) {
				die('<h3>Error:</h3> Failed to retrieve user details.');
			}
			...
			$email = $user_info->email ?? ''; // Twitter may not provide email
	
			if (empty($email)) {
				$this->session->data['warning'] = 'Twitter login requires email verification.';
				$this->response->redirect($this->url->link("account/login", "", 'SSL'));
			}
			...
			if ($customer_info) {
				if (!$customer_info['status']) {
					$this->session->data['warning'] = 'Your account is not approved.';
					$this->response->redirect($this->url->link("account/login", "", 'SSL'));
				}
			} else {
				...
				if ($customer_group_info['approval']) {
					$this->db->query("
						INSERT INTO `" . DB_PREFIX . "customer_approval` 
						SET customer_id = '" . (int)$customer_id . "', type = 'customer', date_added = NOW()
					");
				}
			}
			...
			if ($this->customer->login($email, '', true)) {
				...
				die('<h3>Error:</h3> Customer login failed.');
			}
		} else {
			die('<h3>Error:</h3> Missing oauth_verifier. Twitter did not grant access.');
		}
	}
	//-----------------------------------------------------------
	...
	public function callback() {
        // Debug log to verify callback hit
        if ($this->config->get('config_error_display')) {
            $this->log->write('Facebook Callback Hit');
