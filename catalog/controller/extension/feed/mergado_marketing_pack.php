...
class ControllerExtensionFeedMergadoMarketingPack extends Controller {
  ...
  public function index() {
    ...
    //extension status
    $this->extension_status = $this->mergado->isActive();
  ...
  private function createProductFeed() {
    // Start timer
    $this->model['logger']->log($this->action . '_start', array(), $this->attrs['s'], $this->attrs['l'], $this->attrs['c']);

    //product limit
    $this->limit = $this->config->get($this->extension_fullname . '_product_feed_limit');
    ...
    //product offset
    $this->offset = !empty($state) &&  $state['current_offset'] != $this->items_count ? $state['current_offset'] : 0; 
    ...
    if(!is_numeric($this->limit) || $this->offset == 0) {
      ...
      $output .= '<CHANNEL xmlns="http://www.mergado.com/ns/1.10">';
      $output .= '<GENERATOR>mergado.opencart.marketingpack.' . str_replace('.', '_', MERGADO_VERSION) . '</GENERATOR>';
      $output .= '<LINK>' . $this->getStoreUrl(). '</LINK>';
    } 

    foreach ($products as $product) {

      if(!$this->isListedInStore($product['product_id'], $this->attrs['s'])) { continue; } //skip unlisted products in store
      ...
      foreach ($options_data as $option_data) {
        ...
        $output .= '<SHOP_PAIRING_ID>' . $option_data['id'] . '</SHOP_PAIRING_ID>';
        ...
        $output .= '<URL><![CDATA[' . $this->replaceStoreURL($this->createProductURL($product['product_id'], "")) . ']]></URL>';
        ...
        foreach($categories as $cat) {
          $catname = $this->getFullCategory($cat['category_id']);
          if(!empty($catname)) {
              $output .= '<CATEGORY><![CDATA[' . $catname . ']]></CATEGORY>';
          }
        }

        $output .= '<PRICE>' . $this->getPrice($product['price'], 0, $option_data['delta_price']) . '</PRICE>';
        $output .= '<PRICE_VAT>' . $this->getPrice($product['price'], 0, $option_data['delta_price'], $product['tax_class_id']) . '</PRICE_VAT>';
        if(isset($product['special']) && intval($product['special']) > 0) {
            $output .= '<PRICE_DISCOUNT>' . $this->getPrice($product['price'], $product['special'], $option_data['delta_price']) . '</PRICE_DISCOUNT>';
            $output .= '<PRICE_DISCOUNT_VAT>' . $this->getPrice($product['price'], $product['special'], $option_data['delta_price'], $product['tax_class_id']) . '</PRICE_DISCOUNT_VAT>';
        }
        ...
        $output .= '<SHIPPING_SIZE><![CDATA[' . $product['length'] . ' x ' . $product['width'] . ' x ' . $product['height'] . ' '. $this->model['main']->getLengthClass($product['length_class_id'], $this->languages[$this->attrs['l']]['language_id']) . ']]></SHIPPING_SIZE>';
        $output .= '<SHIPPING_WEIGHT><![CDATA[' . ($product['weight'] + $option_data['delta_weight']) . ' '. $this->model['main']->getWeightClass($product['weight_class_id'], $this->languages[$this->attrs['l']]['language_id']) . ']]></SHIPPING_WEIGHT>';
        $output .= '<PRODUCER><![CDATA[' . $product['manufacturer'] . ']]></PRODUCER>';
        $output .= '<ITEMGROUP_ID>' . $product['product_id'] . '</ITEMGROUP_ID>';
        $output .= '<SHOP_PAIRING_PARENT_ID>' . $product['product_id'] . '</SHOP_PAIRING_PARENT_ID>';

        if(!empty($option_data['property'])) {
          for($i = 0; $i < count($option_data['property']); $i++) {
            $output .= '<PARAM>';
            $output .= '<NAME><![CDATA[' . $option_data['property'][$i] . ']]></NAME>';
            $output .= '<VALUE><![CDATA[' . $option_data['property_value'][$i] . ']]></VALUE>';
            $output .= '</PARAM>';
          }
        }
        
        $output .= '<STOCK_QUANTITY>' . $this->stockQuantity($product['quantity'], $option_data['quantity']) . '</STOCK_QUANTITY>';
        $output .= '</ITEM>';
      }
    }
  ...
  private function createCategoryFeed() {
    ...
    //category limit
    $this->limit = $this->config->get($this->extension_fullname . '_category_feed_limit');
    ...
    $this->items_count = count($this->model['main']->getAllCategories($this->languages[$this->attrs['l']]['language_id']), $this->attrs['s']);
    $categories = $this->model['main']->getAllCategories($this->languages[$this->attrs['l']]['language_id'], $this->attrs['s'], $filter);

    $output = "";
    if(!is_numeric($this->limit) || $this->offset == 0) {
      $output  .= '<?xml version="1.0" encoding="utf-8"?>';
      $output .= '<CHANNEL xmlns="http://www.mergado.com/ns/category/1.7">';
      $output .= '<GENERATOR>mergado.opencart.marketingpack.' . str_replace('.', '_', MERGADO_VERSION) . '</GENERATOR>';
      $output .= '<LINK>' . $this->getStoreUrl(). '</LINK>';
    } 
    ...
    foreach ($categories as $category) {
      if(!empty($category['name'])) {
          ...
          $output .= '<CATEGORY_NAME><![CDATA[' . $category['name'] . ']]></CATEGORY_NAME>';
          $output .= '<CATEGORY><![CDATA[' . $this->getFullCategory($category['category_id'], ' | ') . ']]></CATEGORY>';
          $output .= '<CATEGORY_URL><![CDATA[' . $this->replaceStoreURL($this->createCategoryURL($this->getFullCategoryPath($category['category_id']))) . ']]></CATEGORY_URL>';
          $output .= '<CATEGORY_QUANTITY>' . $this->getItemQuantityInCategory($category['category_id']) . '</CATEGORY_QUANTITY>';
          $output .= '<CATEGORY_DESCRIPTION><![CDATA[' . $this->plainText($category['description']) . ']]></CATEGORY_DESCRIPTION>';
          $output .= '<CATEGORY_MIN_PRICE_VAT>' . $this->getItemMinPriceInCategory($category['category_id']) . '</CATEGORY_MIN_PRICE_VAT>';
          $output .= '<CATEGORY_MAX_PRICE_VAT>' . $this->getItemMaxPriceInCategory($category['category_id']) . '</CATEGORY_MAX_PRICE_VAT>';
          $output .= '</ITEM>';
      }
    }
  ...
  private function createHeurekaAvailabilityFeed() {
    ...
    $this->limit = $this->config->get($this->extension_fullname . '_heureka_availability_feed_limit');
  ...
  private function  validateSecurityToken() {
    ...
    if(!isset($this->attrs['h'])) {
      ...
      echo -2;
      exit;
    } elseif ( isset($this->attrs['a']) && isset($this->attrs['s']) && isset($this->attrs['l']) && isset($this->attrs['c']) ) { 
      if( $this->attrs['a'] == 'pf' && md5($this->model['main']->getMergadoToken() . '_pf_' . $this->attrs['s'] . '_' . $this->attrs['l'] . '_' . $this->attrs['c']) != $this->attrs['h']) { //check product feed hash token
        echo -2;
        exit;
      } elseif ( $this->attrs['a'] == 'cf' && md5($this->model['main']->getMergadoToken() . '_cf_' . $this->attrs['s'] . '_' . $this->attrs['l'] . '_' . $this->attrs['c']) != $this->attrs['h']) { //check category feed hash token
        echo -2;
        exit;
      }
  ...
  private function setFeedHash() {
      $this->feed_hash = md5((isset($this->attrs['a']) ? $this->attrs['a'] : '') . '_' . (isset($this->attrs['s']) ? $this->attrs['s'] : '') . (isset($this->attrs['l']) ? '_' . $this->attrs['l'] : '') . (isset($this->attrs['c']) ? '_' . $this->attrs['c'] : ''));
  }

  private function createFeed($feed_name, $data){
    $root_dir = DIR_APPLICATION.'../mergado/';
    $tmp_dir = DIR_APPLICATION.'../mergado/tmp/';

    //create root dir
    if(!file_exists($root_dir)) {
      if(!mkdir($root_dir, 0755, true)) {
          $this->model['logger']->log('error', 'failed to create mergado folder');
      } else {
          chmod($root_dir, 0755);
      }
    }
    ...
    if(is_numeric($this->limit)) {
      if(!file_exists($tmp_dir)) {
        if(!mkdir($tmp_dir, 0755, true)) {
            $this->model['logger']->log('error', 'failed to create mergado temp folder');
        } else {
            chmod($tmp_dir, 0755);
        }
      }
    ...
    if(!$is_final_feed && !$is_feed_chunk) { //error during xml/chunk creation
      ...
      ), $this->attrs['s'], isset($this->attrs['l']) ? $this->attrs['l'] : null , isset($this->attrs['c']) ? $this->attrs['c'] : null);

      $args = array(
        'change_date' => date('Y-m-d H:i:s', $final_time->getTimestamp()),
        'last_creation_date' => null,
        'current_offset' => !is_numeric($this->limit) || ($this->limit + $this->offset) > $this->items_count ? $this->items_count : ($this->limit + $this->offset),
        'items_count' => $this->items_count,
        'status' => -1
      );
      ...
      $args = array(
        'change_date' => date('Y-m-d H:i:s', $final_time->getTimestamp()),
        'last_creation_date' => $is_final_feed ? date('Y-m-d H:i:s', $final_time->getTimestamp()) : null,
        'current_offset' => !is_numeric($this->limit) || ($this->limit + $this->offset) > $this->items_count ? $this->items_count : ($this->limit + $this->offset),
        'items_count' => $this->items_count,
        'status' => 1
      );
  ...
  private function replaceStoreURL($orig_url){
    ...
    if(MERGADO_DEBUG_MODE) {
        $this->model['logger']->log('debug', 'replace_url - input: url => ' . $orig_url . ', store => ' . $this->attrs['s']);
    }
  ...
  private function getStoreUrl() {
    ...
    return !empty($this->stores) && isset($this->attrs['s']) && $this->attrs['s'] != 0 ? $this->stores[$index]['url'] : $this->base_url;
  }
  ...
  private function getStores() {
    ...
    $this->stores[] = array( //default store
      ...
      'url'      => isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')) ? HTTPS_SERVER : HTTP_SERVER,
      'ssl'      => isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')) ? 1 : 0
    );
  ...
  private function isListedInStore($product_id, $store_id) {

      $product_stores = $this->model['main']->getProductStores($product_id);

      return in_array($store_id, $product_stores);
  }
  ...
  private function generate_options_data($product_id, $name) {
    ...
    if(!empty($options)){
      $input_array = array();
      $input_array[] = array("id"=>$product_id, "name"=>$name, "delta_price"=>0, "delta_weight"=>0, 'property' => array(), 'property_value' => array(), 'quantity' => array());
      ...
      foreach ($options as $option) {
        if(!empty($option['product_option_value'])){
          ...
          if(isset($option['required']) && $option['required'] == 0) {
            $output_array[] = array("id"=>$product_id, "name"=>$name, "delta_price"=>0, "delta_weight"=>0, 'property' => array(), 'property_value' => array(), 'quantity' => array());
          }
          foreach ($input_array as $input_array_member) {
            if($option['type'] == 'checkbox') { //TO DO all variants for checkbox
              ...
              foreach ($option['product_option_value'] as $product_option_value_iterator) {
                $this->generate_option($option, $product_option_value_iterator, $input_array_member, $output_array);
              }
  ...
  private function generate_option($option, $product_option_value_iterator, $input_array_member, &$output_array) {
    ...
    array_push($output_array, array("id"=>$temp_id, "name"=>$temp_name, "delta_price"=>$temp_price, "delta_weight"=>$temp_weight, 'property' => $properties, 'property_value' => $properties_value, 'quantity' => $quantity));
...
//    }
  ...
  private function getPrice($price , $special, $delta_price=0, $tax_class_id = null) {
    ...
    if(!is_null($tax_class_id)) {
      return number_format((float)$this->tax->calculate($computed_price, $tax_class_id), (int)$decimal_places, '.', '');
    } else {
  ...
  private function getCountyId(){
    ...
    if (strpos($this->attrs['l'], '-') !== false) { //for cases like 'cs-cz', 'pl-pl'
    ...
    } else { //for cases like 'czech', 'english', 'cz', 'sk' etc.
  ...
  private function stockQuantity($quantity, $options_quantity) {
    ...
    if(!empty($options_quantity)) { //if product have options get quantity from option with the lowest quantity
  ...
  private function getItemMaxPriceInCategory($category_id) {
    $price_data = $this->model['main']->getItemMinMaxPriceInCategory($category_id, $this->attrs['s'], 'MAX');
  ...
  private function getItemMinPriceInCategory($category_id) {
    $price_data = $this->model['main']->getItemMinMaxPriceInCategory($category_id, $this->attrs['s'], 'MIN');
