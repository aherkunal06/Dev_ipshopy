<?php 
class ControllerCommonHomeDiwali extends Controller {
    public function index() {
        $this->load->model('catalog/home_diwali');
        $this->load->model('tool/image');


 $data['First_banner'] = 'image\catalog\festival\banner1.png';
        $data['First_banner_link'] = $this->url->link('product/category', 'path=3107');
        

        $catgeory_id = [2167,2104,1887,3106,2141];
$catgeory = $this->model_catalog_home_diwali->getCategoryInfoWithPriceAndRating($catgeory_id);

$data['catgeory'] = [];
foreach ($catgeory as $catgeorys) {
    $data['catgeory'][] = [
        'category_id' => $catgeorys['category_id'],
        'name' => $catgeorys['name'],
        'thumb'=> $catgeorys['image']
                    ? $this->model_tool_image->resize($catgeorys['image'], 200, 200)
                    : $this->model_tool_image->resize('catalog/banners/logo/logo1.png', 130, 130),
        'href' => $this->url->link('product/category', 'path=' . $catgeorys['category_id']),
    ];
}

// Debug test
// var_dump($data['catgeory']);
// exit;
        ...
        // -------------------- Section 1 --------------------
        $category_id1 = 2123;
        $subcategories1 = $this->model_catalog_home_diwali->getSubCategoriesWithPriceAndRating($category_id1);
        ...
        // -------------------- Section 2 --------------------
        // $category_id2 = 1887;
        // $subcategories2 = $this->model_catalog_home_diwali->getSubCategoriesWithPriceAndRating($category_id2);
 ...
 $category_group_settings = [
    'groups' => [
      ...
      $data['ladies_group'] = $group_blocks['groups']['Beauty_group']['group_combined'] ?? '';
        ...
        //   $data['related_product'] = $this->load->view('common/related_product');
        //    $data['related_category'] = $this->load->view('common/related_category');
		$data['popular_brands']= $this->load->controller('common/popular_brands' );
        // return $this->load->view('common/home_diwali', $data);
			$data['header'] = $this->load->controller('common/header');
			$data['most_view'] = $this->load->controller('common/most_view');

         $this->response->setOutput($this->load->view('common/home_diwali', $data));
    }
    ...
    public function loadMore()
    {
        ...
        foreach ($paged_sections as $name => $category_id) {
            ...
            $buildSubcats = function ($category_id, $card_limit) use ($img_w, $img_h) {
                $items = [];
                $subcats = $this->model_catalog_home_diwali->getAllSubCategoriesByParent($category_id, 0, $card_limit);
            ...
            $buildProducts = function ($category_id, $card_limit) use ($img_wp, $img_hp) {
                ...
                $prods = $this->model_catalog_home_diwali->getProductsByCategory($category_id, 0, $fetch_limit);
