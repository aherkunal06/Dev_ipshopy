...
class Controllervendorreport extends Controller {
    ...
    public function index() {
        ...
        $log_file = DIR_LOGS . 'cron_daily_report.txt';
        
        // Log the start of the cron job
        file_put_contents($log_file, 'Cron job started at ' . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
        
        if (!isset($this->request->get['cron_key']) || $this->request->get['cron_key'] !== 'asdfg12345') {
            // Log unauthorized access attempt
            file_put_contents($log_file, 'Unauthorized access attempt at ' . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
            
            exit('Unauthorized access');
        }
        
        // Log that the cron key was validated successfully
        file_put_contents($log_file, 'Cron key validated at ' . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
        ...
        $totals = $this->model_vendor_report->TotalOrderStatus();
        ...
        foreach ($statuses as $status) {
            $status_report = $this->model_vendor_report->getOrdersByStatus($status);  // Pass the status argument
        ...
        foreach ($vendor_data as $vendor_id => $seller) {
            ...
            $html_message = $this->load->view('vendor/email_report', $data);  // Render the email body
        ...
        // Log the success of sending emails
        file_put_contents($log_file, 'Daily vendor order report emails sent at ' . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
    ...
    private function generateExcelReport($processing_orders, $delivered_orders, $complete_orders, $label_orders, $breached_orders, $canceled_orders, $reversed_orders, $returnintransit_orders, $returndelivered_orders, $rto_orders, $rtointransit_orders, $rtodelivered_orders) {
        ...
        if (!empty($processing_orders) || !empty($delivered_orders) || !empty($complete_orders) || !empty($label_orders) || !empty($breached_orders) || !empty($canceled_orders)) {
            $filenameForward = DIR_REPORT . 'forward_order_report_' . date('Y-m-d') . '.csv';
            $fileForward = fopen($filenameForward, 'w');
            // Write headers
            fputcsv($fileForward, ['Order ID', 'Product Name', 'Quantity', 'Total Amount', 'Shipping Method', 'Tracking No.', 'Status', 'Date Added']);
        ...
        if (!empty($reversed_orders) || !empty($returnintransit_orders) || !empty($returndelivered_orders)) {
            $filenameBackward = DIR_REPORT . 'backward_order_report_' . date('Y-m-d') . '.csv';
            $fileBackward = fopen($filenameBackward, 'w');
            // Write headers
            fputcsv($fileBackward, ['Order ID', 'Product Name', 'Quantity', 'Total Amount', 'Shipping Method', 'Tracking No.', 'Status', 'Date Added']);
        ...
        if (!empty($rto_orders) || !empty($rtointransit_orders) || !empty($rtodelivered_orders)) {
            $filenameRTO = DIR_REPORT . 'rto_order_report_' . date('Y-m-d') . '.csv';
            $fileRTO = fopen($filenameRTO, 'w');
            // Write headers
            fputcsv($fileRTO, ['Order ID', 'Product Name', 'Quantity', 'Total Amount', 'Shipping Method', 'Tracking No.', 'Status', 'Date Added']);
    ...
    private function deleteOldReports() {
        foreach (['forward', 'backward', 'RTO'] as $type) {
            foreach (glob(DIR_REPORT . "{$type}_order_report_*.csv") as $file) {
    ...
    public function secureDownload() {
        ...
        foreach (glob(DIR_REPORT . "forward_order_report_*.csv") as $old_forwardfile) {
            if (filemtime($old_forwardfile) < time() - 86400) {
                unlink($old_forwardfile);
            }
        }
        foreach (glob(DIR_REPORT . "backward_order_report_*.csv") as $old_backwardfile) {
            if (filemtime($old_backwardfile) < time() - 86400) {
                unlink($old_backwardfile);
            }
        }
        foreach (glob(DIR_REPORT . "RTO_order_report_*.csv") as $old_RTOfile) {
            if (filemtime($old_RTOfile) < time() - 86400) {
                unlink($old_RTOfile);
            }
        }

        $filesforward = glob(DIR_REPORT . "forward_order_report_*.csv");
        $filesbackward = glob(DIR_REPORT . "backward_order_report_*.csv");
        $filesRTO = glob(DIR_REPORT . "RTO_order_report_*.csv");
