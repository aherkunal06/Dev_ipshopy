<?php
class ControllerVendorOutForDeliveryOrders extends Controller {
    private $error = array();
    ...
    public function index() {
        ...
        $this->load->language('vendor/out_for_delivery_orders');
    ...
    public function outfordeliveryview()
	{
		...
		$data['cancel'] = $this->url->link('vendor/out_for_delivery_orders');
		...
		$this->response->setOutput($this->load->view('vendor/out_for_delivery_view', $data));
	}
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'] = array(
            ...
            array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('vendor/out_for_delivery_orders', '', true))
        );

        $data['out_for_delivery_orders'] = array();
        ...
        $vendor_id = $this->vendor->getId();
        $order_total = $this->model_vendor_out_for_delivery_orders->getTotalOutForDeliveryOrders($vendor_id, $filter_data);
        $results = $this->model_vendor_out_for_delivery_orders->getOutForDeliveryOrders($vendor_id, $filter_data);
        ...
        foreach ($results as $result) {
            ...
            $data['out_for_delivery_orders'][] = array(
                ...
                'view' => $this->url->link('vendor/out_for_delivery_orders/outfordeliveryview', 'order_id=' . $result['order_id'], true)
            );
        }
        ...
        $pagination->url = $this->url->link('vendor/out_for_delivery_orders', $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('vendor/out_for_delivery_orders', $data));
    }
}
?>
