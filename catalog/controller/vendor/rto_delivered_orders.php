<?php
class ControllerVendorRtoDeliveredOrders extends Controller {
    private $error = array();
    ...
    public function index() {
        ...
        $this->load->language('vendor/rto_delivered_orders');
    ...
    public function rtodeliveredview()
	{
		...
		$data['cancel'] = $this->url->link('vendor/rto_delivered_orders');
		...
		$this->response->setOutput($this->load->view('vendor/rtodelivered_view', $data));
	}
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'] = array(
            ...
            array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('vendor/rto_delivered_orders', '', true))
        );

        $data['rto_delivered_orders'] = array();
        ...
        $vendor_id = $this->vendor->getId();
        $order_total = $this->model_vendor_rto_delivered_orders->getTotalRtoDeliveredOrders($vendor_id, $filter_data);
        $results = $this->model_vendor_rto_delivered_orders->getRtoDeliveredOrders($vendor_id, $filter_data);
        ...
        foreach ($results as $result) {
            ...
            $data['rto_delivered_orders'][] = array(
                ...
                'view' => $this->url->link('vendor/rto_delivered_orders/rtodeliveredview', 'order_id=' . $result['order_id'], true)
            );
        }
        ...
        $pagination->url = $this->url->link('vendor/rto_delivered_orders', $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('vendor/rto_delivered_orders', $data));
    }
}
?>
