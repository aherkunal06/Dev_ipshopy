<?php
class ControllerVendorLatestOrder2 extends Controller
{
...
$product_total = $this->model_vendor_vendor->getTotalOrderProductsByOrderId($order['order_id'], $vendor_id);
$vorder_total = $this->model_vendor_vendor->getvendorOrdertotal($vendor_id, $order['order_id']);
			...
			$products = $this->model_vendor_vendor->getOrderProducts($order['order_id']);
			...
			$data['orders'][] = array(
				...
				'products' => $product_data,  // Pass product data (name & image) to the view
				'view' => $this->url->link('vendor/latestorder2/letestview', '&order_id=' . $order['order_id'])
			);
		}
		...
		return $this->load->view('vendor/latestorder2', $data);
	}
	...
	public function letestview()
	{
		...
		if (!empty($order_info)) {
			...
			$data['invoice'] = $this->url->link('vendor/latestorder2/invoice', '&order_id=' .  $order_info['order_id']);
			/* 07-10-2020 */
			$data['shipping'] = $this->url->link('vendor/latestorder2/shipping', '&order_id=' .  $order_info['order_id']);
			/* 07-10-2020 */
			// Payment Address
					...
					if ($option['type'] != 'file') {
						...
						if ($upload_info) {
							$option_data[] = array(
								...
								'type'  => $option['type'],
								'href'  => $this->url->link('vendor/latestorder2/download', '&code=' . $upload_info['code'], true)
							);
						}
					}
				}
			...
			$pagination->url = $this->url->link('vendor/latestorder2/letestview', '' . '&order_id=' . $this->request->get['order_id'] . '&page={page}', true);
	...
	public function invoice()
	{
		...
		$store_infos = $this->model_vendor_store->getViewStore($vendor_id);
		...
		// ----------------------------------------------------------------------------------------------------------------------------------------------
					...
					if (isset($courier_result->row['courier_charges'])) {
						$product['courier_charges'] = $courier_result->row['courier_charges'];
					} else {
		...
		$this->response->setOutput($this->load->view('vendor/order_invoice2', $data));
	}
	/* 01 02 2020 */
	public function shipping()
	{
		...
		foreach ($orders as $order_id) {
			...
			// ✅ Determine GST Type  ----------------------------------------------------------------
			$vendor_zone_id = $store_infos['zone_id'];
			$shipping_zone_id = $order_info['shipping_zone_id'];

			if ($vendor_zone_id == $shipping_zone_id) {
			$gst_type = 'cgst_sgst'; // Local/Zonal
			} else {
			$gst_type = 'igst'; // National
			}
			$data['gst_type'] = $gst_type;
			$data['gst_rate'] = 18.00; // Example GST rate, adjust as needed
...
//                        // ✅ Step 1: GST Type logic
// $vendor_zone_id = $store_infos['zone_id']; // Vendor's state
// $customer_zone_id = $order_info['shipping_zone_id']; // Customer's shipping state

// if ($vendor_zone_id == $customer_zone_id) {
//     $gst_type = 'cgst_sgst'; // Local/Zonal
// } else {
//     $gst_type = 'igst'; // National
// }
...
// $data['gst_type'] = $gst_type;
		...
		// ----------------------------------------------------------------------------------------------------------------------------------------------
						...
						$product_data[] = array(
							...
							'hsn_code' => $product['hsn_code'], // ✅ HSN Code
                             'gst_rate' => $product['gst_rate'], // ✅ GST Rate
