...
foreach ($product_option['product_option_value'] as $product_option_value) {
    $this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET 
        ...
        length = '" . (float)$product_option_value['length'] . "', 
        width = '" . (float)$product_option_value['width'] . "', 
        height = '" . (float)$product_option_value['height'] . "', 
        volumetric_weight = '" . (float)$product_option_value['volumetric_weight'] . "',
        mrp = '" . (float)$product_option_value['mrp'] . "'
    ");
}
...
// 		}
	...
	public function editProduct($product_id, $data) {
	    ...
	    // added the code changes on the 21-05-2025
	    // added hsn code and gst rate columns into the following query on 27-04-2025
        ...
        gst_rate = '" . $this->db->escape($data['gst_rate']) . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', edited_by = '" . $this->db->escape($data['edited_by']) . "',  tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW(), volumetric_weight = '" . (float)$data['volumetric_weight'] . "', payment_method = '" . $this->db->escape($data['payment_method']) . "' WHERE product_id = '" . (int)$product_id . "'");
        
        // start the code from here changes on the 24-05-2025
			$query = $this->db->query("SELECT status FROM `" . DB_PREFIX . "product` WHERE product_id = '" . (int)$product_id . "'");
...
// 		adding local zonal and national courier charges 
        ...
        courier_free_price = '" . $this->db->escape(isset($data['courier_free_price']) ? $data['courier_free_price'] : null) . "', 
        date_added = NOW()");
// 		adding local zonal and national courier charges end 
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_added_by'])) {
			$sql .= " AND added_by LIKE '" . $this->db->escape($data['filter_added_by']) . "%'";
		}

		if (!empty($data['filter_edited_by'])) {
			$sql .= " AND edited_by LIKE '" . $this->db->escape($data['filter_edited_by']) . "%'";
		}
	...
	public function getProductOptionValue($product_id, $product_option_value_id) {
		$query = $this->db->query("SELECT pov.option_value_id, ovd.name, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.points, pov.points_prefix, pov.actual_weight, pov.actual_weight_prefix FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_id = '" . (int)$product_id . "' AND pov.product_option_value_id = '" . (int)$product_option_value_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
...
// 	Priyanka Madam Additions - 11/05/2025
...
// faq ends 
	// 	warranty , return , replace starts
