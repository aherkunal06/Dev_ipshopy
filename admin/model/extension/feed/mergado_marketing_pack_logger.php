...
class ModelExtensionFeedMergadoMarketingPackLogger extends Model {
    ...
    public function exportCSV() {
        ...
        fputcsv($out, array('id', 'date','label', 'log', 'store id', 'lang code', 'currency'));
        foreach($this->getLogs() as $log) {
            fputcsv($out, array($log['log_id'],$log['log_date'], $log['log_label'], $log['log_msg'], $log['store_id'], $log['lang_code'], $log['currency']));
        }
        fclose($out);
    }

    public function clearAll() {
        return $this->db->query("TRUNCATE TABLE " . DB_PREFIX . self::DB_LOG_TABLE );
    }
