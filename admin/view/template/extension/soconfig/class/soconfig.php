...
class Soconfig{
	...
	public function js_out($JSExcludes = array(), $position = 'header'){
    $optimizeJSExclude = array();
    if (is_array($JSExcludes)) {
        foreach ($JSExcludes as $JSExclude) {
            $optimizeJSExclude[] = $JSExclude['namecss'];
        }
    }
    ...
    $optimizeCSSExclude = [];
    if (!empty($CSSExcludes)) {
        foreach ($CSSExcludes as $CSSExclude) {
            $optimizeCSSExclude[] = $CSSExclude['namecss'];
        }
    }

    $css_files_to_out = isset($this->css_files[$position]) ? $this->css_files[$position] : [];

    if ($this->get_settings('cssminify', '0') == 1) {
        if (empty($CSSExcludes) || !array_intersect($this->css_files[$position], $optimizeCSSExclude)) {
        ...
        // Output all CSS files without minification
